...
class IMRPhenomD(TaylorF2):
    ...
    def phenom_d_htilde(
        ...
        Psi, _ = self.phenom_d_phase(Mf, mass_1, mass_2, eta, eta2, chi1, chi2, xi)
    ...
    def phenom_d_int_amp(
        self, Mf, eta, eta2, Seta, chi1, chi2, chi12, chi22, xi, fRD=None, fDM=None
    ):
        # merger ringdown
        if fRD is None or fDM is None:
            fRD, fDM = self.fring_fdamp(eta, eta2, chi1, chi2)
        ...
        v3, d2 = self.phenom_d_mrd_amp(Mf3, eta, eta2, chi1, chi2, xi, fRD, fDM)
        v2 = (torch.ones_like(Mf).mT * self.AmpIntColFitCoeff(eta, eta2, xi)).mT
        ...
        amp = (
            delta_0 + Mf * delta_1 + Mf**2 * (delta_2 + Mf * delta_3 + Mf**2 * delta_4)
        )
        Damp = delta_1 + Mf * (2 * delta_2 + 3 * Mf * delta_3 + 4 * Mf**2 * delta_4)
        return amp, Damp

    def phenom_d_mrd_amp(self, Mf, eta, eta2, chi1, chi2, xi, fRD=None, fDM=None):
        # merger ringdown
        if fRD is None or fDM is None:
            fRD, fDM = self.fring_fdamp(eta, eta2, chi1, chi2)
    ...
    def phenom_d_inspiral_amp(self, Mf, eta, eta2, Seta, xi, chi1, chi2, chi12, chi22):
        SetaPlus1 = 1 + Seta
        ...
        prefactors_one = (
            (chi1 * (81 * SetaPlus1 - 44 * eta) + chi2 * (81 - 81 * Seta - 44 * eta))
            * PI
        ) / 48.0
    ...
    def phenom_d_phase(
        ...
        int_phase_rd, int_Dphase_rd = self.phenom_d_int_phase(fRDJoin, eta, eta2, xi)
    ...
    def phenom_d_mrd_phase(self, Mf, eta, eta2, chi1, chi2, xi, fRD=None, fDM=None):
